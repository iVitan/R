...
shandong_district_data <- read.csv(
  "G:/WorkPlace/R/R/Rstudy/shandong/District.csv",
...
mymap <- readOGR(
  "G:/WorkPlace/R/R/Rstudy/shandong/map.shp",
  stringsAsFactors= FALSE
  )
...
shandong_city_map_data <- readOGR(
  "G:/WorkPlace/R/R/Rstudy/shandong/山东省.json",
  encoding = "utf-8",
  use_iconv = TRUE,
  stringsAsFactors=FALSE
  ) 
...
shandong_city_data <- read.csv(
  "G:/WorkPlace/R/R/Rstudy/shandong/City.csv",
  stringsAsFactors=FALSE
)
...
#市级地图数据——shandong_city_map_data（含城市指标及边界点经纬度数据）
#市级指标数据——shandong_city_data（含城市中心点经纬度）

#县级地图数据——shandong_district_map_data（含17个地级市指标及边界点经纬度）
#县级指标数据——shandong_district_data（含137个行政县指标及中心点经纬度和五个指标数据，GDP规模、GDP增速，人均GDP增速，预算规模、预算增速）。
...
ggplot(shandong_city_map_data,aes(SUM_GDPScale))+
  geom_histogram()
summary(shandong_city_map_data$SUM_GDPScale)
...
######1.4.2 GDP规模&人均GDP增速（对应PDF文件第4张图）######
##请根据1.4.1代码，完成山东省各县区GDP规模&人均GDP增速热力填充图
...
#####1.4.3 公共预算收入规模&增速(公共预算增速)分布（对应PDF文件第7张图）#####
##请根据1.4.1代码，完成山东省各县区GDP规模&人均GDP增速热力填充图
...
####1.4.4 各县GDP规模&公共预算收入规模的分布情况（对应PDF文件第6张图）####
#对于每一个点：点的大小表示GDP规模，点的颜色深浅表示公共预算收入
