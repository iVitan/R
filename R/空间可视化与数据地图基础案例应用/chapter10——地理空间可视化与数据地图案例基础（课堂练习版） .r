...
china_map <- readOGR("F:/R/第十章/Rstudy/CHN_adm",stringsAsFactors = FALSE)
...
mydata2   <- read.csv("F:/R/第十章/Rstudy/geshengzhibiao.csv",stringsAsFactors = FALSE)   #读取业务指标数据(csv格式)
...
province_city <- read.csv("F:/R/第十章/Rstudy/chinaprovincecity.csv",stringsAsFactors = FALSE)  #读取省会城市坐标
...
mydata2   <- read.csv("F:/R/第十章/Rstudy/geshengzhibiao.csv",stringsAsFactors = FALSE)
...
newdata <- read.csv("F:/R/第十章/Rstudy/President.csv",stringsAsFactors=FALSE,check.names=FALSE)  #从新读入美国大选的选票数据集：
...
world_map <-readOGR("F:/R/rstudy/wold_map/World_region.shp",stringsAsFactors=FALSE)
