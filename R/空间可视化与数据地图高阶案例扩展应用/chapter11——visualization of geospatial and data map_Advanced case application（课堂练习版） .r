...
china_shp <-readOGR("F:/R/第九章/CHN_adm/bou2_4p.shp",stringsAsFactors = FALSE)    
...
ggplot()+
  ...
  geom_linerange(data=province_data,aes(x=jd -5,ymin =wd,ymax = wd + 0.7*N15/max(N15,N16)*5),size=3,color="#5B88A0",alpha=0.8)+#添加竖状条形图，根据图片的刻度范围设置柱子的高度
  geom_linerange(data=province_data,aes(x=jd + 5,ymin = wd,ymax = wd + 0.7*N16/max(N15,N16)*5),size=3,color="#FB882C",alpha=0.8)+#添加竖状条形图，根据图片的刻度范围设置柱子的高度
...
province_city <- read.csv(
  "F:/R/第十章/Rstudy/chinaprovincecity.csv",
  stringsAsFactors = FALSE,
  check.names=FALSE
  )
...
city_data2$Full_scale<- as.numeric(city_data2$Full)*2 #对Full列数据标准化并乘以2作为饼图的半径大小
...
#baidumap加载使用
library(baidumap)
options(baidumap.key ='vitan')#其中???为个人在百度地图平台申请的账号名（https://github.com/badbye/baidumap）
library("showtext")
...
mydata1 <- read.csv("F:/R/第十章/Rstudy/geshengzhibiao.csv",stringsAsFactors = FALSE,check.names=FALSE)
...
source("F:/R/11web_city_adress.r",encoding = "utf-8")#source是执行r脚本文件,web_city_adress.r是在百度Api接口抓取指定地市经纬度的脚本程序
address <- GetJD(city_list)#使用脚本程序里定义的函数GetJD获取city_list对应城市的经纬度数据


address$lonx <- address[address$address =="长沙","jd"]#增加列：长沙的经度做为起点的x坐标
address$laty <- address[address$address =="长沙","wd"]#增加列：长沙的纬度做为起点的y坐标
...
ggplot()+
  ...
  geom_point(data=address,aes(x=lon,y=lat,xend=lonx,yend=laty),size=0.3,colour="#FF6833")+#增加直线段图层
  geom_point(data=address,aes(x=lon,y=lat,size=0.3),shape=21,fill="#ED7D31",col="#E02939",alpha=.6)+#增加散点图层
  geom_point(data=address,aes(x=112.97935,y=28.21347),shape=21,size=8,fill=NA,col="steelblue")+#增加长沙所在城市的散点
  guides(fill=FALSE)+
  coord_map("polyconic") +
...
source("F:/Rstudy/web_city_adress.r",encoding = "utf-8")
...
ggplot()+
  ...
  geom_point(data=address,aes(x=jd,y=wd),size=0.3,colour="#FF6833")+#增加路径图（按address城市先后顺序自动绘制路径）
  geom_point(data=address,aes(x=lng,y=lat,size=0.3),shape=21,fill="#ED7D31",col="#E02939",alpha=.6)+
  guides(fill=FALSE)+
  coord_map("polyconic")+
  scale_size_area(max_size=8)+ 
  theme_void() %+replace%
...
ggplot()+
  ...
  geom_point(data=address,aes(x=lng,y=lat),size=0.3,colour="#FF6833")+#绘制路径图
  geom_point(data=address,aes(x=lng,y=lat,size=???),shape=21,fill="#ED7D31",col="#E02939",alpha=.6)+#绘制散点图
  guides(fill=FALSE)+
  coord_map("polyconic")+
  scale_size_area(max_size=8)+ 
  theme_void() %+replace%
...
province_city <- read.csv(
  "D:/Rstudy/chinaprovincecity.csv",
  stringsAsFactors = FALSE,
  check.names = FALSE
) 
...
ggplot()+
  geom_polygon(data=china_map,aes(x=long,y=lat,group=group),fill="white",colour="grey60") +
  geom_segment(data=city_data,aes(x=start_long,y=Start_lat,xend=???,yend=End_lat,size=0.3),colour="black")+
  coord_map("polyconic") + 
